...
2. Delivered locally, non-duplicable: `/research/ap-payment-run-exclusion-evidence-research` has one route-local handoff to `/services/payment-run-preparation`. It explains that a Philippines-based support role prepares exclusion records and routes questions, while authorized employees verify bank details, approve exceptions, and release payments. Do not add a second CTA.
3. Next verified-absent pair: `/research/ap-invoice-receipt-date-evidence-research` has no route-local link to `/services/month-end-ap-close-support` in the current production build. Its rendered owner is `app/research-sep7-batch.ts`; the shadowed August 14 record was removed so a later data-owned handoff cannot be written to an unused record. The route asks readers to separate receipt dates from service, approval, and posting dates, so a single data-owned handoff to close-evidence preparation is the next candidate. Keep cutoff and accounting decisions with the finance owner; first locate a route-owned sentence about preserving receipt-date evidence rather than adding a generic CTA.
