...
3. Next verified-absent pair: `/research/ap-invoice-receipt-date-evidence-research` has no route-local link to `/services/month-end-ap-close-support` in the current production build. The route asks readers to separate receipt dates from service, approval, and posting dates, so a single data-owned handoff to close-evidence preparation is the next candidate. Keep cutoff and accounting decisions with the finance owner; first locate a route-owned sentence about preserving receipt-date evidence rather than adding a generic CTA.
