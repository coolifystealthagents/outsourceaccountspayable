...
// batch constant.
const aug13Topics = [
  ['ap-invoice-tax-field-review', 'AP invoice tax field review checklist', 'invoice tax field review', 'Check tax fields against the source invoice and route uncertain treatment to the finance owner before entry.', '2026-08-13'],
  ['ap-invoice-payment-reference-check', 'Invoice payment reference check for AP teams', 'invoice payment reference check', 'Keep invoice and payment references connected so a reviewer can trace a proposed item back to the source record.', '2026-08-13'],
  ['ap-vendor-legal-name-crosscheck', 'Vendor legal-name cross-check for AP support', 'vendor legal-name cross-check', 'Compare the invoice name with approved vendor records and document differences before a packet moves forward.', '2026-08-13'],
  ['ap-invoice-approval-owner-confirmation', 'Invoice approval owner confirmation process', 'invoice approval owner confirmation', 'Confirm the correct approver from current company records and stop when ownership is unclear or outdated.', '2026-08-13'],
  ['ap-receipt-date-sequence-review', 'Receipt date sequence review for AP matching', 'receipt date sequence review', 'Compare order, receipt, and invoice dates to surface timing gaps for an accountable owner.', '2026-08-13'],
  ['ap-invoice-freight-charge-review', 'Invoice freight charge review checklist', 'invoice freight charge review', 'Separate freight charges from goods or services and route unsupported differences for review.', '2026-08-13'],
  ['ap-vendor-credit-note-index', 'Vendor credit note index for AP reconciliation', 'vendor credit note index', 'Index credit notes against supplier statements and invoices so open credits do not disappear from the review queue.', '2026-08-13'],
  ['ap-invoice-duplicate-attachment-review', 'Invoice duplicate attachment review', 'invoice duplicate attachment review', 'Identify repeated files and altered versions while retaining the original attachment for reviewer comparison.', '2026-08-13'],
  ['ap-ap-close-accrual-request-log', 'AP close accrual request log', 'AP close accrual request log', 'Record open service evidence and owner responses that finance may need when preparing close decisions.', '2026-08-13'],
  ['ap-invoice-department-code-review', 'Invoice department code review for AP support', 'invoice department code review', 'Check department coding against approved records and isolate judgment calls for the finance reviewer.', '2026-08-13'],
  ['ap-vendor-statement-missing-invoice-list', 'Vendor statement missing-invoice list', 'vendor statement missing invoice list', 'Turn statement gaps into specific invoice requests with owners, dates, and retrievable evidence.', '2026-08-13'],
  ['ap-invoice-approval-decision-log', 'Invoice approval decision log fields', 'invoice approval decision log', 'Capture who decided, what record was reviewed, when the decision occurred, and what remains unresolved.', '2026-08-13'],
  ['ap-payment-batch-exception-review', 'Payment batch exception review for AP support', 'payment batch exception review', 'Prepare a review list for missing approvals, changed details, and duplicate risks before an authorized release.', '2026-08-13'],
  ['ap-invoice-supplier-account-crosscheck', 'Invoice supplier-account cross-check', 'invoice supplier-account cross-check', 'Compare supplier account identifiers with approved records and pause unexpected changes for independent review.', '2026-08-13'],
  ['ap-open-invoice-owner-aging-review', 'Open invoice owner aging review', 'open invoice owner aging review', 'Show how long an invoice has waited, who owns the next step, and what evidence is still missing.', '2026-08-13'],
  ['ap-invoice-approval-absence-escalation', 'Invoice approval absence escalation rules', 'invoice approval absence escalation', 'Use a backup path for unavailable approvers without treating silence as approval.', '2026-08-13'],
  ['ap-close-invoice-completeness-review', 'AP close invoice completeness review', 'AP close invoice completeness review', 'Check the close population for source invoices, status, owner notes, and unresolved exceptions.', '2026-08-13'],
  ['ap-vendor-document-expiry-tracker', 'Vendor document expiry tracker for AP teams', 'vendor document expiry tracker', 'Track expiring vendor records and route renewals while keeping acceptance with the authorized owner.', '2026-08-13'],
  ['ap-invoice-amount-tolerance-review', 'Invoice amount tolerance review process', 'invoice amount tolerance review', 'Make amount differences visible against documented rules instead of allowing an assistant to approve exceptions.', '2026-08-13'],
  ['ap-ap-reconciliation-unresolved-items', 'AP reconciliation unresolved-items log', 'AP reconciliation unresolved items', 'Keep reconciling differences assigned with evidence, next action, and a dated finance decision.', '2026-08-13'],
  ['ap-invoice-source-document-index', 'AP invoice source-document index', 'invoice source-document index', 'Connect source files, invoice records, and exception notes so every prepared item remains reviewable.', '2026-08-13'],
  ['ap-vendor-payment-hold-review', 'Vendor payment hold review checklist', 'vendor payment hold review', 'Document the reason, source evidence, owner, and release decision for a payment hold.', '2026-08-13'],
] as const;

export const aug13BlogPublicationDates: Record<string, string> = {
  ...Object.fromEntries(aug13Topics.map(([slug,,,, date]) => [slug, date])),
};
