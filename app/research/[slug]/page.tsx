...
export default async function ResearchPost({params}:{params:Promise<{slug:string}>}){
  ...
  return <><Header/><main className="section article-page"><article className="container guide-article"><p className="eyebrow">Research · Published <time dateTime={p.published}>{p.published}</time></p><h1>{p.title}</h1><p className="lead">{p.excerpt}</p><div className="article-meta">Methodology-led research · {p.citations.length} primary sources · Updated <time dateTime={p.modified}>{p.modified}</time></div><img src={p.thumbnail} alt={`${p.title} research illustration`} width="1200" height="630" style={{width:'100%',height:'auto'}}/>
