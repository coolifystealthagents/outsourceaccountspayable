...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const p=researchPosts.find(x=>x.slug===slug);return p?{title:`${p.title} | ${site.brand}`,description:p.excerpt,openGraph:{images:[p.thumbnail]}}:{}}
...
export default async function ResearchPost({params}:{params:Promise<{slug:string}>}){
  ...
  return <><Header/><main className="section article-page"><article className="container guide-article"><p className="eyebrow">Research · Published {p.published}</p><h1>{p.title}</h1><p className="lead">{p.excerpt}</p><div className="article-meta">Methodology-led research · {p.citations.length} primary sources · Updated {p.modified}</div><img src={p.thumbnail} alt={`${p.title} research illustration`} width="1200" height="630" style={{width:'100%',height:'auto'}}/>
