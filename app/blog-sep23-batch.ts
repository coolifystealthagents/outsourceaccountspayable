...
const makeStages = (labels: string[]): [string,string][] => labels.map((label, index) => [label, [
  'Capture the source record, stable identifier, date, amount, entity, and current status before changing the queue.',
  'Compare the governing record with the invoice or payment evidence and record both values when they disagree.',
  'Classify the exact exception, its financial effect, the missing evidence, and the employee owner who can decide it.',
  'Prepare one focused question with source links, a response deadline, and no unsupported recommendation.',
  'Apply only the authorized response, preserve the previous value, and repeat duplicate, approval, and access checks.',
  'Close the item only when the final record, approval, ledger state, and next owner are independently reviewable.',
][index] ?? 'Keep the source, owner, decision, and final status together in a reviewable record.']);

const topics: Topic[] = [
  {slug:'invoice-number-normalization-control',title:'Normalize supplier invoice numbers without creating duplicates',excerpt:'Build a controlled invoice-number format that preserves the supplier value, exposes collisions, and supports duplicate review.',keyword:'invoice number normalization control',service:'duplicate-invoice-review',serviceLabel:'duplicate invoice review',scenario:'invoice numbers that vary by spaces, punctuation, prefixes, or leading zeros',owner:'AP control or finance owner',records:'invoice image, supplier account, original invoice number, normalized comparison key, duplicate results, and owner decision',example:'A supplier sends AB-00127 while the ERP contains AB00127. The processor keeps the printed value and uses a separate comparison key instead of overwriting either record.',stages:makeStages(['Preserve the supplier value','Define the comparison rule','Search within the right scope','Review collisions','Post the authorized value','Retain the duplicate trail'])},
  {slug:'split-invoice-review-workflow',title:'Review split invoices before approval and payment',excerpt:'Detect invoices divided by amount, date, entity, or purchase-order line and route them without assuming the split is valid or evasive.',keyword:'split invoice review workflow',service:'duplicate-invoice-review',serviceLabel:'duplicate invoice review',scenario:'two or more related invoices that may represent one commercial obligation',owner:'purchasing or finance owner',records:'invoice set, purchase order, contract, receipts, approval thresholds, prior postings, and supplier correspondence',example:'Three invoices for $9,800 arrive against one $29,400 service milestone. AP groups the evidence and asks the owner to confirm the billing basis before separate approvals proceed.',stages:makeStages(['Group related documents','Compare the commercial basis','Check thresholds and timing','Trace receipts and prior bills','Route the split decision','Recheck the final set'])},
  {slug:'credit-memo-approval-routing',title:'Route supplier credit memos to the right invoice and owner',excerpt:'Connect each credit memo to the correct supplier account, invoice, dispute, entity, and approval path before applying it.',keyword:'credit memo approval routing',service:'vendor-statement-reconciliation',serviceLabel:'vendor statement reconciliation',scenario:'a supplier credit that lacks a clear invoice reference or application instruction',owner:'AP accounting or finance owner',records:'credit memo, original invoice, supplier statement, dispute record, entity and currency, ledger, and approval',example:'A $3,250 credit cites a customer account but no invoice. The processor narrows the possible open items and leaves application authority with the finance owner.',stages:makeStages(['Capture the credit as issued','Find candidate invoices','Check entity and currency','Reconcile the supplier statement','Obtain application authority','Verify the remaining balance'])},
  {slug:'blocked-vendor-payment-review',title:'Review a blocked vendor payment before the next payment run',excerpt:'Separate vendor holds, invoice blocks, compliance stops, and bank-data exceptions so a payment is not released on a vague unblock request.',keyword:'blocked vendor payment review',service:'payment-run-preparation',serviceLabel:'payment run preparation',scenario:'a payment proposal excluded because the supplier or invoice carries a system block',owner:'treasury, compliance, or finance owner',records:'invoice, vendor record, block code and history, approval, exception evidence, payment proposal, and owner response',example:'An approved invoice is excluded because the supplier record is blocked after a bank-change request. AP documents both facts but does not remove the block or reuse the old approval.',stages:makeStages(['Identify every active block','Trace the reason and actor','Separate invoice and vendor status','Revalidate sensitive changes','Request controlled release','Verify the payment outcome'])},
  {slug:'advance-payment-reconciliation',title:'Reconcile a supplier advance payment to later invoices',excerpt:'Track deposits and prepayments from authorization through supplier application, invoice matching, and residual balance.',keyword:'supplier advance payment reconciliation',service:'vendor-statement-reconciliation',serviceLabel:'vendor statement reconciliation',scenario:'a supplier deposit or advance that should offset one or more later invoices',owner:'finance or project owner',records:'advance request, approval, payment result, contract, supplier statement, invoices, credits, and ledger balance',example:'A $20,000 deposit funds a project, followed by invoices totaling $18,600. The reconciliation keeps the $1,400 residual visible rather than treating the invoices as fully settled.',stages:makeStages(['Establish the advance balance','Confirm the governing terms','Match later invoices','Track partial applications','Resolve differences with the owner','Carry or clear the residual'])},
  {slug:'purchase-order-unit-of-measure-mismatch',title:'Resolve a purchase-order unit-of-measure mismatch',excerpt:'Compare invoice, order, receipt, conversion evidence, and pricing when units such as cases, eaches, hours, or kilograms disagree.',keyword:'purchase order unit of measure mismatch',service:'three-way-match-support',serviceLabel:'three-way match support',scenario:'an invoice quantity or rate stated in a different unit from the purchase order or receipt',owner:'purchasing or receiving owner',records:'invoice, purchase-order line, receipt, item master, conversion source, price terms, and approval',example:'The order is for 40 cases, the receipt shows 480 eaches, and the invoice bills 480 units. AP must prove the 12-per-case conversion before calling the quantities equal.',stages:makeStages(['Label every quantity and unit','Find the controlled conversion','Recalculate extended prices','Check receipts and tolerances','Route missing or changed conversions','Repeat the match'])},
  {slug:'supplier-portal-invoice-download-log',title:'Create a reliable supplier-portal invoice download log',excerpt:'Record portal access, invoice identity, download time, file integrity, and queue handoff without treating a portal visit as proof of receipt.',keyword:'supplier portal invoice download log',service:'ap-inbox-management',serviceLabel:'AP inbox management',scenario:'invoices retrieved from supplier portals rather than delivered to the AP mailbox',owner:'AP process or system owner',records:'portal account, access approval, invoice metadata, file hash, download log, intake record, and exception history',example:'A portal lists an invoice revised twice under the same number. The processor logs each file identity and routes the latest labeled version without deleting the earlier evidence.',stages:makeStages(['Use named least-privilege access','Record the portal snapshot','Identify each downloaded file','Prevent duplicate intake','Handle revisions and failures','Reconcile the portal and queue'])},
  {slug:'invoice-attachment-version-control',title:'Control invoice attachment versions in the AP inbox',excerpt:'Distinguish originals, corrected invoices, supporting files, and unrelated attachments so the approved packet points to the right source.',keyword:'invoice attachment version control',service:'ap-inbox-management',serviceLabel:'AP inbox management',scenario:'an email thread containing multiple invoices or revised attachments with similar names',owner:'AP process or finance owner',records:'original messages, attachment names and hashes, invoice metadata, correction request, approval, and posting reference',example:'A supplier replies with invoice-final.pdf after invoice-revised.pdf, but both share the same number. AP retains both, labels their receipt order, and asks which document replaces the earlier request.',stages:makeStages(['Preserve the original message','Fingerprint every attachment','Classify document purpose','Link corrections explicitly','Point approval to one version','Retain the superseded trail'])},
  {slug:'early-payment-discount-review',title:'Review an early-payment discount before taking it',excerpt:'Validate the invoice terms, eligible amount, deadline, approval, and payment feasibility before a discount enters the payment proposal.',keyword:'early payment discount review',service:'payment-run-preparation',serviceLabel:'payment run preparation',scenario:'an invoice offering a discount if payment is completed by a stated date',owner:'treasury or finance owner',records:'invoice, contract or purchase terms, receipt date, approval time, discount calculation, payment calendar, and bank result',example:'An invoice offers 2% if paid within ten days, but the clock may run from invoice date or receipt date. AP presents both dates and the contract instead of choosing the favorable interpretation.',stages:makeStages(['Capture the stated terms','Confirm the governing start date','Calculate the eligible amount','Check approval and cutoff timing','Obtain the finance decision','Reconcile discount and payment'])},
  {slug:'payment-run-currency-cutoff',title:'Set a currency cutoff for payment-run preparation',excerpt:'Organize currency-specific funding, rate, bank, and approval deadlines without letting AP choose rates or release payments.',keyword:'payment run currency cutoff',service:'payment-run-preparation',serviceLabel:'payment run preparation',scenario:'a payment run containing invoices in currencies with different funding and bank deadlines',owner:'treasury or finance owner',records:'approved invoices, currency totals, bank calendar, funding evidence, rate source, payment proposal, and release result',example:'USD and EUR items share an approval run, but the EUR bank cutoff is two hours earlier. The queue shows separate readiness deadlines and leaves funding and rate decisions with treasury.',stages:makeStages(['Group approved items by currency','Record timezone-aware cutoffs','Separate funding and invoice status','Control rate evidence','Freeze the proposal for approval','Reconcile every currency result'])},
  {slug:'dormant-vendor-reactivation-checklist',title:'Use a checklist before reactivating a dormant vendor',excerpt:'Treat a request to reuse an inactive supplier as a controlled onboarding event with independent identity, ownership, tax, and payment-data checks.',keyword:'dormant vendor reactivation checklist',service:'vendor-onboarding-administration',serviceLabel:'vendor onboarding administration',scenario:'a supplier record marked inactive or unused for a long period that someone wants to transact with again',owner:'vendor-master, compliance, or finance owner',records:'inactive vendor record, reactivation request, identity and tax documents, approved contacts, bank verification, ownership review, and authorization',example:'A buyer asks AP to reactivate a supplier last used three years ago and provides new bank details in the same email. The request is logged, but the details follow the independent verification path.',stages:makeStages(['Confirm business need and requester','Review the dormant record history','Refresh identity and tax evidence','Verify contacts and payment data','Obtain vendor-master authorization','Monitor the first transaction'])},
  {slug:'ap-close-carryforward-register',title:'Build an AP close carryforward register for unresolved items',excerpt:'Carry open invoice, credit, receipt, approval, and reconciliation exceptions into the next close with a named owner and preserved evidence.',keyword:'AP close carryforward register',service:'month-end-ap-close-support',serviceLabel:'month-end AP close support',scenario:'unresolved AP items that cannot be completed by the current accounting close',owner:'AP close or controllership owner',records:'open-item extract, exception packet, period decision, accrual or adjustment reference, owner, due date, and next-close outcome',example:'A service invoice lacks acceptance evidence at cutoff. The register shows the invoice and open question, while controllership separately records any period decision and the next review date.',stages:makeStages(['Freeze the close snapshot','Classify the unresolved item','Link the period decision','Name the next owner and date','Review aging before the next close','Close with final evidence'])},
];
...
export const sep23BlogPosts = topics.map(topic => ({slug:topic.slug,title:topic.title,excerpt:topic.excerpt,minutes:12}));

export const sep23BlogDetails = Object.fromEntries(topics.map(topic => [topic.slug, {
  thumbnail: `/blog-thumbnails/sep23-2026/${topic.slug}.svg`,
